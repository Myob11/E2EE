...
for i in {1..30}; do
	if sudo "${COMPOSE_CMD[@]}" exec -T postgres pg_isready -U postgres -d postgres >/dev/null 2>&1; then
		break
	fi
...
printf "%s\n" \
	...
	"\\gexec" | sudo "${COMPOSE_CMD[@]}" exec -T postgres psql -U postgres -d postgres -v ON_ERROR_STOP=1
